...
OPEN_DIR   = Path("dataset/train_dataset/open")
CLOSED_DIR = Path("dataset/train_dataset/closed")
TMP_DIR    = Path("eye_split")        
MODEL_DIR  = Path("model")
MODEL_PATH = MODEL_DIR / "eye_model.keras"
...
def build_split():
    ...
    for folder, label in [(OPEN_DIR, "open"), (CLOSED_DIR, "closed")]:
...
def train(epochs=EPOCHS, batch_size=BATCH_SIZE):
    ...
    print("\n[Eye CNN] Building split folders...")
    build_split()

    print("[Eye CNN] Creating generators...")
...
def plot(h1, h2, test_acc, title):
    ...
    plt.tight_layout()
    out = MODEL_DIR / "eye_cnn_history.png"
...
if __name__ == "__main__":
    print("  Eye CNN Trainer — EfficientNetB0")
